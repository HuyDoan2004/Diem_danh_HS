...
class DatabaseHandler:
    ...
    def get_session_by_date(self, date):
        ...
        try:
            ...
            # Eager-load schedule để dùng ngoài phạm vi session (tránh DetachedInstanceError)
            return (
                session.query(AttendanceSession)
                .options(joinedload(AttendanceSession.schedule))
                .filter(
    ...
    def check_in(self, session_id, student_id, confidence, check_in_time=None, status=None):
        """Điểm danh check-in

        Args:
            session_id: ID buổi học
            student_id: ID trong bảng students
            confidence: độ tin cậy nhận diện
            check_in_time: thời gian điểm danh (mặc định = now)
            status: 'present' hoặc 'late'; nếu None sẽ tự tính như cũ
        """
        session = self.get_session()
        try:
            ...
            # Determine status (present or late) nếu chưa được truyền từ bên ngoài
            if status is None:
                ...
                # Mặc định cho phép đi trễ 10 phút nếu không cấu hình từ ngoài
    ...
    def get_attendance_by_session(self, session_id):
        ...
        session = self.get_session()
        try:
            # Eager-load student để có thể truy cập att.student.* sau khi session đóng
            return (
    ...
    def get_student_attendance_history(self, student_id, start_date=None, end_date=None):
        ...
        session = self.get_session()
        try:
            # Join với AttendanceSession một lần và filter theo khoảng ngày
            query = (
                session.query(Attendance)
                .join(AttendanceSession, Attendance.session_id == AttendanceSession.id)
                .options(
                    joinedload(Attendance.session).joinedload(AttendanceSession.schedule),
    ...
    def get_attendance_statistics(self, student_id):
        ...
        try:
            ...
            # 'present' = đúng giờ, 'late' = đi trễ (không tính vào tỷ lệ điểm danh)
            ...
            return {
                ...
                'late': late,
                'absent': absent,
                # Tỷ lệ điểm danh chỉ tính buổi 'present' (đúng giờ)
