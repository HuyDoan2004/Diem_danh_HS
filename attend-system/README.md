...
def mark_attendance(student_id, session_id, confidence):
    ...
    elif current_time <= class_start_time + timedelta(minutes=15):
        status = 'late'     # Muộn 15 phút
    else:
        status = 'absent'   # Vắng (quá muộn)
...
# Chọn option 2: Initialize database
...
- Nhận diện bằng model đã training
- Tu dong check-in khi nhận diện thành công
- Hiển thị tên và MSSV trên màn hình
- Quan ly session diem danh
...
Chức năng:
- Tổng quan: Hiển thị metrics tổng thể
- Quản lý học sinh: Thêm, sửa, xóa, import Excel
- Xem diem danh: Chi tiet theo phiên hoc
- Báo cáo: Thống kê theo hoc sinh va thoi gian
- Quản lý lịch học
