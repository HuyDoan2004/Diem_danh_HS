...
def show_overview(db):
    ...
    with col3:
        # Chỉ tính 'present' là đã điểm danh đúng giờ
        present_count = len([a for a in today_attendances if a.status == 'present'])
        st.metric("Đã điểm danh (đúng giờ)", present_count)
