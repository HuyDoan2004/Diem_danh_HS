...
class ModelEvaluator:
    ...
    def cross_validation_analysis(self, X, y, cv=5):
        ...
        # Nếu chỉ có 1 lớp thì không thể train SVM / nhiều classifier phân biệt lớp
        # => bỏ qua phần cross-validation để tránh lỗi và vẫn giữ các kết quả khác.
        unique_classes = np.unique(y)
        if len(unique_classes) < 2:
            print("\n[WARNING]  Cross-validation skipped: only 1 class available.")
            print("          Add more students/classes to enable multi-class CV.")
            return {}
